...
// parses only on Node 24 and up, while this package supports Node 22. See the
// note at the top of `core.test.js`.
...
test("produces the same bytes as the thread pool", async () => {
  const threaded = new Compiler({ fonts: TEXT_FONTS });
  try {
    const processed = pooled();
    try {
      ...
      // Isolation must not change output. If it did, `pool.mode` would silently
      // be a rendering decision rather than a scheduling one.
  ...
  try {
    assert.throws(
      ...
      (error) => {
        ...
        // Falling back to an in-process compile would be worse than failing:
        // the caller asked for isolation and would not get it.
...
test("a runaway compile is killed, and the pool recovers", async () => {
  const c = pooled({ size: 1, timeoutMs: 400 });
  try {
