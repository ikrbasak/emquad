...
"@emquad/binding#test":   { "cache": false },
"@emquad/core#test":      { "dependsOn": ["build", "@emquad/fonts#build", "@emquad/binding#test"], "cache": false }
```

**`build` deliberately has no `^build`.** Nothing here needs another package's build output in
order to build — tsdown externalizes `@emquad/binding`, and the declarations it type-checks against
are generated by napi-rs and committed. Wiring the dependency anyway dragged
`@emquad/binding`'s several-minute release LTO link into every task that transitively touched it,
which was most of them.

That mattered most in the test path. `@emquad/binding`'s *test* script builds its own addon with
...
with a debug build. The suites depend on `@emquad/binding#test` instead, which keeps the ordering
